...
/* Ngôn ngữ ở phía SERVER — đọc từ cookie ngay trong lần dựng HTML đầu.
   ...
   ⚠ Đọc cookie làm cả cây route thành ĐỘNG (không dựng tĩnh được nữa). Với hệ
   này là chấp nhận được — mọi màn đều sau đăng nhập và đều đọc dữ liệu theo
   phiên, chỉ có `/dang-nhap` trước đây còn tĩnh. Đổi lại là
   `<html lang>` đúng ngay từ byte đầu, thứ trình đọc màn hình dựa vào.

   ⚠ Từ Next 15, `cookies()` TRẢ VỀ PROMISE nên các hàm ở đây đều async — cùng
   lý do đã ghi ở `lib/auth/session.ts`. */
