/* Chữ ký tay — hàm thuần, không import React, không chạm document.

   Port từ commercial-console.html: initPad / clearPad / loadSigImage / padData,
   và vòng lặp điểm ảnh của stripBg.

   Bản cũ trộn phép tính với DOM trong cùng một hàm. Ở đây tách phần tính ra: cỡ
   khung vẽ theo mật độ điểm ảnh, phép co ảnh chữ ký cho vừa khung, ngưỡng xoá
   nền trắng, điều kiện nhận tệp ảnh. Phần chạm canvas nằm ở ChuKyPad.tsx.

   Năm chỗ đừng gỡ khi sửa:

   1. Một chấm không phải chữ ký. Bản cũ chỉ bật cờ drawn trong pointermove nên
      chạm nhầm một cái thì hợp đồng không bị coi là đã ký; giữ nguyên ý đó, một
      nét phải có từ 2 điểm.
   2. devicePixelRatio có thể bằng 0 hoặc không có (trên server). Nhân bừa ra
      khung 0×0 là canvas tàng hình, và toDataURL() trên canvas rỗng chiều ném
      lỗi ở một số trình duyệt.
   3. Co ảnh dùng min chứ không phải max. Ảnh chữ ký chụp bằng điện thoại rất
   ...
   4. Xoá nền trắng phải xét đủ ba kênh màu. Bỏ kênh xanh lam thì mực highlight
      vàng (250, 250, 10) cũng bị xoá theo.
   5. Tệp tải lên phải kiểm. Ảnh gốc 8 MB nhét vào payload dưới dạng base64 là
      hợp đồng phình gấp bốn và backend trả 413.

   Hai hàm viSao… trả LyDo (khoá i18n + chỗ điền) chứ không trả câu tiếng Việt;
   màn gọi t(lyDo.khoa, lyDo.thamSo). Dùng LyDo chứ không phải khoá trần vì có
   nhánh mang con số tính ngay trong hàm ("Ảnh 8.0 MB, vượt mức 2 MB."). */
...
/** Một nét phải có từng này điểm mới tính là có vẽ. */
export const DIEM_TOI_THIEU_MOT_NET = 2;

/** Đã có chữ ký chưa. Một chấm (nét 1 điểm) không tính. */
...
/** Cỡ thật của canvas (backing store) từ cỡ CSS và mật độ điểm ảnh.
    ...
    dpr không hợp lệ (0, âm, NaN, undefined khi không có window) rơi về 1. Cạnh
    nhỏ hơn 1 điểm ảnh cũng kéo lên 1: canvas rỗng chiều vừa không vẽ được vừa
    làm toDataURL() ném lỗi. */
...
/** Co ảnh cho vừa trọn trong khung rồi căn giữa, kiểu object-fit: contain.

    Math.min chứ không phải Math.max. Trả null khi ảnh hoặc khung có cạnh bằng
    0: chia cho 0 ra Infinity, drawImage() nhận vào thì hoặc ném lỗi hoặc vẽ ra
    khoảng trắng. */
...
/** Ảnh vào hợp đồng dưới dạng base64, phình thêm ~33%. 2 MB là đủ rộng cho
    một ảnh chữ ký chụp bằng điện thoại. */
export const KICH_THUOC_ANH_TOI_DA = 2 * 1024 * 1024;

/** Vì sao chưa nạp được tệp ảnh chữ ký — null nghĩa là nạp được.

    Trả lý do chứ không phải boolean: người ở quầy cần biết phải làm gì tiếp,
    không phải một ô chọn tệp im lặng không nhận. */
...
/** Chữ ký phải là ảnh nhúng, không phải đường dẫn.
    ...
    Một đường dẫn https:// trỏ tới ảnh chữ ký là chữ ký có thể đổi hoặc biến mất
    sau khi hợp đồng đã ký: máy chủ ảnh chết là hợp đồng trắng ô ký, mà đổi tệp
    ở đầu kia thì không ai biết. Chứng từ phải tự chứa. */
